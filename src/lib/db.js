...
-- Пятнадцати секунд не хватило: 26.09 в 15:00:01 три полосы стартовали одновременно и
-- одна из них упала на синхронизации реестра, не начав сбор. Минута дешевле потерянного
-- гео — ждать всё равно дешевле, чем падать.
...
const MIGRATIONS = [
  ...
  ['metrics_keyword_geo', 'ext_impressions_prev', 'REAL'],
...
// Повтор на SQLITE_BUSY для идемпотентных операций. Нужен на старте процесса: три полосы
// поднимаются в одну секунду, каждая пишет реестр, и одна может не дождаться даже минуты,
// если в этот момент идёт контрольная точка WAL. Повторять можно только то, что безопасно
// выполнить дважды, — поэтому это не общий механизм, а явная обёртка по месту.
export function retryBusy(fn, tries = 5, waitMs = 3000) {
  for (let i = 1; ; i++) {
    try { return fn(); } catch (e) {
      const busy = e && (e.code === 'SQLITE_BUSY' || /database is locked/i.test(String(e.message)));
