...
export async function run({ geo, date }) {
  ...
  log(`  AppRadar 3: out/appradar3.html (${(Buffer.byteLength(html) / 1048576).toFixed(1)} МБ), гео ${geos.filter((g) => g.date).length}, кандидатов ${rows.length}, ниш ${niches.length}`);
