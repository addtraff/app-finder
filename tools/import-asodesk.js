...
//
// Первое: «Calculating» — это НЕ ноль. Сервис считает объём не мгновенно, и на первой
// выгрузке 24.09 из 1 021 ключа ядра США числа были у 614. Записать остальным ноль
// означало бы объявить «спроса нет» там, где мы просто ещё не знаем. Такие строки
// пропускаются целиком, а уже известное значение ими не затирается.
//
...
const insHist = d.prepare(
  `INSERT INTO raw_external_keyword_hist
     (geo, keyword, snapshot_date, source, daily_impressions, difficulty, apps_ranked, brand_app, imported_at)
   VALUES (?,?,?,?,?,?,?,?,?)
...
const insCur = d.prepare(
  `INSERT INTO raw_external_keyword_planner
     (geo, keyword, avg_monthly_searches, imported_at, daily_impressions, competition_index,
      apps_ranked, brand_app, source, measured_at)
   VALUES (?,?,?,?,?,?,?,?,?,?)
...
d.transaction(() => {
  for (const r of rows.slice(1)) {
    ...
    if (dif != null) withDif++;
    if (brand) brands++;
    if (imp == null && dif == null && apps == null && !brand) continue;
    insHist.run(geo, kw, date, source, imp, dif, apps, brand, now);
    // Месячный объём — дневные показы × 30: поле в таблице месячное, а сервис даёт дневное.
    insCur.run(geo, kw, imp == null ? null : Math.round(imp * 30), now, imp, dif, apps, brand, source, date);
  }
})();

log(`Asodesk → ${geo}, снимок ${date}, файл ${path.basename(file)}`);
log(`  строк ${seen}: объём известен у ${withImp} (из них нулевых ${zero}), ещё считается у ${calculating}, сложность у ${withDif}, бренд держит запрос у ${brands}`);
...
log(`  ядро ${geo}: ${core} ключей, объём известен у ${covered} (${core ? Math.round(covered / core * 100) : 0} %)`);
